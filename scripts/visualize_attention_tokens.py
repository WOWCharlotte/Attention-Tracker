...
def merge_result_metadata(record: dict, result_rows: list[dict] | None) -> dict:
    if not result_rows:
        return record
...
def main() -> None:
    ...
    record = merge_result_metadata(record, result_rows)
    os.makedirs(os.path.dirname(args.output) or ".", exist_ok=True)
    with open(args.output, "w", encoding="utf-8") as f:
        f.write(render_html(record, args.input))
    print(args.output)
